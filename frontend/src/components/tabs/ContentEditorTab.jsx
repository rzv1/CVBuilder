...
import contentSchema from '@/frontend/src/schema/content-schema.json';
import styleSchema from '@/frontend/src/schema/style-schema.json';
...
// Configure Monaco YAML support globally for content and style schemas
setDiagnosticsOptions({
  ...
  schemas: [
    {
      uri: 'https://cvbuilder.ai/schemas/content.yaml',
      fileMatch: ['*content*.yaml', 'inmemory://model/content*'],
      schema: contentSchema
    },
    {
      uri: 'https://cvbuilder.ai/schemas/style.yaml',
      fileMatch: ['*style*.yaml', 'inmemory://model/style*'],
      schema: styleSchema
    }
  ]
});

// Register custom DocumentColorProvider for YAML to enable Monaco native Color Picker & Color Decorators
if (!window.__monaco_yaml_color_provider_registered) {
  window.__monaco_yaml_color_provider_registered = true;
  monaco.languages.registerColorProvider('yaml', {
    provideDocumentColors(model) {
      const text = model.getValue();
      const colors = [];
      const regex = /#([0-9a-fA-F]{6}|[0-9a-fA-F]{3}|[0-9a-fA-F]{8})\b/g;
      let match;
    ...
    provideColorPresentations(model, colorInfo) {
      ...
      let hexText = `#${toHex(r)}${toHex(g)}${toHex(b)}`;
      if (alpha < 1) {
...
export default function ContentEditorTab({ cvData, setCvData, styleData, setStyleData, isDevMode }) {

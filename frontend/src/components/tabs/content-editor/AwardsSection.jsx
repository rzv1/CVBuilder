...
export default function AwardsSection({
  ...
  const isOpen = activeSection === 'awards';
          ...
          {awardsData.map((award, awdIdx) => (
            <div
              ...
              className={`p-4 rounded-xl border border-slate-800 bg-slate-900/90 space-y-4 transition-all ${
                draggedItem?.sectionKey === 'awards' && draggedItem?.index === awdIdx
            ...
            >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <Button
                      variant="ghost"
                      size="icon-xs"
                      disabled={awdIdx === awardsData.length - 1}
                      onClick={() => moveItem('awards', awdIdx, 1)}
                      className="h-6 w-6 text-slate-400 hover:text-white disabled:opacity-30"
                    >
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-300">Award Title</label>
                <Input type="text" value={award.title || ''} onChange={(e) => handleAwardChange(awdIdx, 'title', e.target.value)} />
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-slate-300">Issuer / Organization</label>
                  <Input type="text" placeholder="e.g. AWS, Tech Summit" value={award.issuer || ''} onChange={(e) => handleAwardChange(awdIdx, 'issuer', e.target.value)} />
                </div>
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-slate-300">Date Received</label>
                  <Input type="text" placeholder="e.g. 2023" value={award.date || ''} onChange={(e) => handleAwardChange(awdIdx, 'date', e.target.value)} />
                </div>
              </div>

              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-300">Award Description</label>
                <Textarea rows={2} placeholder="Brief summary of why award was conferred..." value={award.description || ''} onChange={(e) => handleAwardChange(awdIdx, 'description', e.target.value)} />
              </div>
            </div>
          ))}
        </div>

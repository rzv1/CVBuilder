...
export async function processChatStream(reqBody, res) {
  const { messages = [], content = {}, style = {}, userId, userName, contextLimit = 2 } = reqBody;
  const N = Math.max(1, parseInt(contextLimit, 10) || 2);

  // Take only the last N messages from chat history as context
  const contextMessages = messages.slice(-N);
  ...
  if (GEMINI_API_KEY) {
    try {
      ...
      const result = streamText({
        ...
        system: systemPrompt,
        messages: formattedMessages.length > 0 ? formattedMessages : [{ role: 'user', content: 'Salut' }]
      });
...
export async function generateChatMessage(reqBody = {}) {
  const { messages = [], content = {}, style = {}, userId, userName, contextLimit = 2 } = reqBody;
  const N = Math.max(1, parseInt(contextLimit, 10) || 2);
  ...
  // Deduct AI credit if user is identified internally
  if (userId || userName) {
    try {
      const user = await getUserById(userId || userName);
      if (user) {
  ...
  if (!GEMINI_API_KEY) {
    throw new Error('Eroare: Cheia API Gemini nu este configurată pe server.');
  }
